...
sys.path.append(drive+'/Astronomy/Python Play/Utils_P3')
#sys.path.append(drive+'\\Astronomy\Python Play\Galaxies')

import matplotlib.pyplot as pl
import pylab
import numpy as np
import SysRespLIB_P3 as SRL
import ConfigFiles_P3 as CF
import PlotUtils_P3 as PU
import GeneralSpecUtils_P3 as GSU
#RETRIEVE ST2000 RESPONSES#####################################################
path=drive+"/Astronomy/Projects/Techniques/InstrumentPerformance-P3/"
...
Mean200linespermm1260mm=GSU.SpectrumAggregation("c:",CLR550_ObsList)
Mean200linespermm1260mm.ComputeAverageandStats()
tmp=SRL.Compute_EWs(path,"Filters/550CLR/1260mm200lpm-550CLR-EW",Mean200linespermm1260mm.MeanSpec,1.0189)
np.savetxt(path+'Filters/550CLR/SystemResponseCLR-1260mm200lpm.txt',Mean200linespermm1260mm.MeanSpec,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")

CLR550_ObsList.load_records(MeasTgt="1260mm100lpm")
Mean100linespermm1260mm=GSU.SpectrumAggregation("c:",CLR550_ObsList)
Mean100linespermm1260mm.ComputeAverageandStats()

CLR550_ObsList.load_records(MeasTgt="135mm100lpm")
Mean100linespermm135mm=GSU.SpectrumAggregation("c:",CLR550_ObsList)
...
Mean200linespermm135mm=GSU.SpectrumAggregation("c:",CLR550_ObsList)
...
MeanNIRtmp=GSU.SpectrumAggregation("c:",NIR685_ObsList)
...
MeanREDtmp=GSU.SpectrumAggregation("c:",RED650_ObsList)
...
MeanGRNtmp=GSU.SpectrumAggregation("c:",GRN550_ObsList)
...
MeanBLUtmp=GSU.SpectrumAggregation("c:",BLU450_ObsList)
...
MeanNUVtmp=GSU.SpectrumAggregation("c:",NUV380_ObsList)
...
########### SCALING IS APPLIED TO THE PLOTS, BUT NOT YET TO THE SAVED
#  TRANSMISSION FILES - SMH 1/29/2022
...
np.savetxt(path+'/Filters/685NIR/685NIR_Transmission.txt',TransNIR,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
...
np.savetxt(path+'/Filters/650RED/650RED_Transmission.txt',TransRED,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
...
np.savetxt(path+'/Filters/550GRN/550GRN_Transmission.txt',TransGRN,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
...
np.savetxt(path+'/Filters/450BLU/450BLU_Transmission.txt',TransBLU,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
...
np.savetxt(path+'/Filters/380NUV/380NUV_Transmission.txt',TransNUV,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
...
#tmp=PU.Draw_with_Conf_Level(MeanBLU1260mm,0.875/1.0244,'C9','BLU 1260mm Response')
#tmp=SRL.Compute_EWs(path,"1260mm200lpm-550BLU-EW",MeanBLU1260mm,0.875/1.0244)
np.savetxt(path+'SystemResponseNUV-1260mm200lpm.txt',MeanNUV1260mm,delimiter=" ",
           fmt="%10.3F %10.7F %10.7F %10.7F")
pl.legend(loc=0,ncol=2, borderaxespad=0.,prop={'size':6})
pl.subplots_adjust(left=0.08, bottom=0.15, right=0.98, top=0.92,
            wspace=None, hspace=None)
